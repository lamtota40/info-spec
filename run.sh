...
cpuinfo=$(cat /proc/cpuinfo | grep MHz | awk '{print int($4 + 0.5)}' | sort | uniq -c | awk '{printf "%dx %.3fGHz,", $1, $2/1000}' | sed 's/,$//')
...
echo -e "Host	 : $domain | $(jq -r '.query' <<< "$cekip")"
echo -e "NS Host	 : $nsdomain"
